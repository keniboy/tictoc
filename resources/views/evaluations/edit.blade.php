...
@section('content')
<div class="mb-6"><h1 class="text-3xl font-bold text-gray-900">Editar Avaliação</h1></div>
<div class="bg-white shadow-sm rounded-lg p-6">
    <form action="{{ route('evaluations.update', $evaluation) }}" method="POST">@csrf @method('PUT')
        <div class="mb-4">
            ...
            <select name="enrollment_id" id="enrollment_id" required class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                ...
                @foreach($enrollments as $enrollment)
                    <option value="{{ $enrollment->id }}" {{ old('enrollment_id', $evaluation->enrollment_id) == $enrollment->id ? 'selected' : '' }}>
        ...
        <div class="mb-4">
            ...
            <input type="number" name="nota" id="nota" value="{{ old('nota', $evaluation->nota) }}" step="0.01" min="0" max="20" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
        ...
        <div class="mb-4">
            ...
            <input type="date" name="data" id="data" value="{{ old('data', $evaluation->data->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
