...
<div class="bg-white shadow-sm rounded-lg p-6">
    <form action="{{ route('courses.update', $course) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="nome" id="nome" value="{{ old('nome', $course->nome) }}" required
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="date" name="data_inicio" id="data_inicio" value="{{ old('data_inicio', $course->data_inicio?->format('Y-m-d')) }}"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                @error('data_inicio')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="date" name="data_fim" id="data_fim" value="{{ old('data_fim', $course->data_fim?->format('Y-m-d')) }}"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                @error('data_fim')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
        ...
        <div class="mb-4">
            ...
            <input type="number" name="valor" id="valor" value="{{ old('valor', $course->valor) }}" step="0.01" min="0" required
                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
            @error('valor')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
