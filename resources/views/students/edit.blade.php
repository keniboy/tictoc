...
@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900">Editar Aluno</h1>
</div>
    ...
    <form action="{{ route('students.update', $student) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="text" name="telemovel" id="telemovel" value="{{ old('telemovel', $student->telemovel) }}"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                @error('telemovel')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="date" name="data_nascimento" id="data_nascimento" value="{{ old('data_nascimento', $student->data_nascimento?->format('Y-m-d')) }}"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <select name="sexo" id="sexo"
                    ...
                    <option value="M" {{ old('sexo', $student->sexo) == 'M' ? 'selected' : '' }}>Masculino</option>
                    <option value="F" {{ old('sexo', $student->sexo) == 'F' ? 'selected' : '' }}>Feminino</option>
                    <option value="O" {{ old('sexo', $student->sexo) == 'O' ? 'selected' : '' }}>Outro</option>
                </select>
                @error('sexo')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="flex items-center">
                <input type="checkbox" name="ativo" id="ativo" value="1" {{ old('ativo', $student->ativo) ? 'checked' : '' }}
